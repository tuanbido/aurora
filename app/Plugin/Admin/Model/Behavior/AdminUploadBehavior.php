...
class AdminUploadBehavior extends ModelBehavior {
	...
	public function afterSave(Model $Model, $created, $options = array()) {
		if (!empty($Model->data[$Model->alias]['file'])) {
			...
			$uploadsPath = classRegistry::init($data['model'])->adminConfig[$uploadType][$data['category_code']]['path'];
			...
			if (!is_dir($dest_dir)) {
				if (!mkdir($dest_dir, 0777, true)) {
					$this->log('AdminPHP: unable to create dir ' . $dest_dir);
				} else {
					chmod($dest_dir, 0777);
				}
			}
	...
	public function beforeDelete(Model $Model, $cascade = true) {
		...
		$uploadsPath = $relModel->adminConfig[$uploadType][$upload['category_code']]['path'];
	...
	public function resizedVersions($Model, $model, $recordId, $sizes, $category_code, $file) {
		...
		$uploadsPath = $RelModel->adminConfig['images'][$category_code]['path'];
		$sourceFile = $uploadsPath . DS . $model . DS . $recordId . DS . $file;
		if (!file_exists($sourceFile)) {
			return false;
		}
		$pathinfo = pathinfo($sourceFile);
		App::import('Vendor', 'Admin.resizeimage');
		...
		if (!is_file($cachedFile)) {
			...
			adminResizeImage($cachedFile, $sizes);
		}
		return $cachedFile;
	}
