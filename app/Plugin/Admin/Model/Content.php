		...
		foreach ($fields as $key => $value) {
			...
			if ($this->fieldUnique($Model, $key)) {
				$rules[$key][] = array(
					'rule' => 'isUnique',
					'message' => sprintf(
						($Model->adminConfig['names']['gender'] == 1) ?
						...
						__($Model->adminConfig['names']['singular'])
					),
					'allowEmpty' => true,
				);
			}
	...
	public function brownieBeforeSave($data, $Model, $Session) {
		...
		foreach ($Model->schema() as $field => $value) {
			if (
				...
				and !in_array($field, $Model->adminConfig['fields']['hide'])
			) {
	...
	*/
	public function makeSave($Model, $data, $fields) {
		...
		if ($Model->adminConfig['sortable']) {
			$fieldList[] = $Model->adminConfig['sortable']['field'];
		}
	...
	public function ownedBeforeSave($data, $Model, $authUserId) {
		...
		if (
			...
			and $Model->adminConfigPerAuthUser[$authModel]['type'] == 'owned'
		) {
	...
	public function fckFields($Model) {
		...
		foreach ($Model->schema() as $field => $metadata) {
			if ($metadata['type'] == 'text' and !in_array($field, $Model->adminConfig['fields']['no_editor'])) {
				$out[] = $field;
			}
		}
		return $out;
	...
	public function defaults($Model) {
		$data = array();
		foreach ($Model->schema() as $field => $value) {
			if (array_key_exists($field, $Model->adminConfig['default'])) {
				$data[$field] = $Model->adminConfig['default'][$field];
	...
	public function reorder($Model, $direction, $id) {
		...
		if ($parent = $Model->adminConfig['parent']) {
		...
		if ($Model->adminConfig['sortable']['direction'] == 'desc') {
			$prev = 'prev'; $next = 'next';
		} else {
			$prev = 'next'; $next = 'prev';
		}
	...
	public function schemaForView($Model) {
		...
		foreach ($schema as $field => $extra) {
			$isForeignKey = $this->adminIsForeignKey($Model, $field);
	...
	public function adminIsForeignKey($Model, $field) {
	...
	public function actions($Model, $record, $permissions) {
		...
		$actionsTitles = array_merge($actionsTitles, array(
			'add' => __d('brownie', $Model->adminConfig['action_labels']['add']),
			'view' => __d('brownie', $Model->adminConfig['action_labels']['view']),
			'edit' => __d('brownie', $Model->adminConfig['action_labels']['edit']),
			'delete' => __d('brownie', $Model->adminConfig['action_labels']['delete']),
			'index' => __d('brownie', $Model->adminConfig['action_labels']['index']),
		));
		...
		foreach ($Model->adminConfig['custom_actions'] as $action => $custom) {
			$matchCondition = true;
	...
	public function findList($Model, $relationData) {
		$parent = $Model->adminConfig['parent'];
	...
	public function relatedModelsForView($Model) {
		...
		if ($Model->adminConfig['images']) {
			$ret['BrwImage'] = array('fields' => '*', 'order' => 'BrwImage.id asc');
		}
		if ($Model->adminConfig['files']) {
	...
	public function relatedModelsForIndex($Model, $paginate) {
		...
		foreach ($paginate['fields'] as $field) {
			$relModel = $this->adminIsForeignKey($Model, $field);
			if ($relModel) {
	...
	public function formatHABTMforView($record, $Model) {
		...
		foreach ($Model->hasAndBelongsToMany as $relModel => $settings) {
			$record['HABTM'][$i] = array(
				'model' => $relModel,
				'name' => $Model->{$relModel}->adminConfig['names']['plural'],
				'data' => array(),
			);
	...
	public function getForExport($Model, $named) {
		...
		if ($Model->adminConfig['export']['replace_foreign_keys']) {
	...
	public function filterConditions($Model, $named, $forData = false) {
		...
		foreach ($Model->schema() as $field => $value) {
			...
			$isRange = (!$forData and (
				...
				or (
					$value['type'] == 'integer'
					and !$this->adminIsForeignKey($Model, $field)
					and empty($Model->adminConfig['fields']['filter'][$field])
				)
			));
			if ($isRange) {
		...
		if (!$forData and method_exists($Model, 'adminFilter')) {
			$filter = $Model->adminFilter($filter, $named);
		}
		return $filter;
	}
	...
	public function getRelatedBrwConfig($Model) {
		...
		foreach ($models as $model => $config) {
			$adminConfigs[$model] = $Model->{$model}->adminConfig;
		}
		return $adminConfigs;
	}
