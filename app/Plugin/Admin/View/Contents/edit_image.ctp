...
<?php
$adding = true;
echo $this->Form->create('Content', array('type' => 'file', 'action' => 'edit_image', 'plugin' => 'admin'));
?>
<h1><?php echo __d('brownie', 'Add image'); ?></h1>
<?php
echo $this->Form->input('model', array('value' => $model, 'type' => 'hidden'));

if(!empty($adminConfig['images'][$categoryCode])){
		$image = $adminConfig['images'][$categoryCode];
		$i = 0;
		echo '
		<fieldset>
