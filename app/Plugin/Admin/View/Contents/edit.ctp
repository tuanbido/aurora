<?php if (!empty($adminConfig['fields']['conditional'])) echo $this->element('js_conditional_fields'); ?>
<div class="form">
<?php
...
<fieldset>
	...
	echo CakeText::insert($action, array('name_singular' => __($adminConfig['names']['singular'])));
	?>
	</legend>
	<?php
	...
	foreach ($fields as $key => $value) {
		...
		if (in_array($value['type'], array('datetime', 'date'))) {
			$params['minYear'] = $adminConfig['fields']['date_ranges'][$key]['minYear'];
			$params['maxYear'] = $adminConfig['fields']['date_ranges'][$key]['maxYear'];
			$params['dateFormat'] = $adminConfig['fields']['date_ranges'][$key]['dateFormat'];
			if ($value['null']) {
				$params['empty'] = '-';
			}
		}

		if (!empty($adminConfig['fields']['legends'][$key])) {
			$params['after'] = $adminConfig['fields']['legends'][$key];
		}
		...
		$params['div'] = array('id' => 'admin' . $model . Inflector::camelize($key));
		$params['label'] = __($adminConfig['fields']['names'][$key]);
		...
		foreach ($related['hasAndBelongsToMany'] as $key => $list) {
			if (!empty($list)) {
				...
				if (count($list) > 5) {
					...
					echo $this->Html->script('/admin/js/jquery.selso');
					echo $this->Html->script('/admin/js/jquery.comboselect');
				}
...
foreach ($uploads as $upload) :
	...
	$continue = false;
	if ($upload == 'Image' and !empty($adminConfig['images'])) {
		$continue = true;
		$uploadConfig = $adminConfig['images'];
	} elseif ($upload == 'File' and !empty($adminConfig['files'])) {
		$continue = true;
		$uploadConfig = $adminConfig['files'];
	}
...
<div class="submit">
	...
	<a href="<?php echo Router::url(array('controller' => 'admin', 'action' => 'index')) ?>" class="cancel">Cancel</a>
</div>
