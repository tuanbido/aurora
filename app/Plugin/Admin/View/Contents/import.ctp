...
<fieldset>
	<legend><?php
	echo CakeText::insert(__d('brownie', 'Import :name_plural'), array('name_plural' => __($adminConfig['names']['plural'])));
	?></legend>
	<?php
	echo $this->Form->input('model', array('value' => $model, 'type' => 'hidden'));
	echo $this->Form->input('file', array('type' => 'file'));
	foreach ($adminConfig['fields']['import'] as $field) {
		...
		echo $this->Form->input($field, array(
			...
			'empty' => '-',
			'label' => $adminConfig['fields']['names'][$field],
		));
	}
	?>
</fieldset>
