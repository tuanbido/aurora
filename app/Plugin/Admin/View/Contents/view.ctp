<div class="view" id="<?php echo $model;?>_view">
	<div class="clearfix">
	<h1><?php  echo __($adminConfig['names']['singular']);?></h1>
	<div class="actions-view">
		<ul class="actions neighbors">
			...
			if (!empty($this->params['named']['back_to'])) {
				$backToUrl = array('plugin' => 'admin', 'controller' => 'contents');
	...
	foreach ($record[$model] as $field_name => $field_value) {
		if (!empty($schema[$field_name]) and !in_array($field_name, $adminConfig['fields']['no_view'])) {
			echo '
			<tr>
				<td class="label">' . __($adminConfig['fields']['names'][$field_name]) . '</td>';
	...
	<?php foreach ($record['HABTM'] as $rel): ?>
		<?php if (!in_array($rel['model'], $adminConfig['hide_related']['hasAndBelongsToMany'])): ?>
		<tr>
...
<div class="admin-<?php echo $uploadKey ?> index">
	<?php foreach ($adminConfig[$uploadKey] as $catCode => $fileCat): ?>
	<div class="<?php echo $uploadKey . '-' . $catCode ?>">
		...
		if ($canAdd) {
			...
			' . $this->Html->link(__d('brownie', 'Add'), array(
				'plugin' => 'admin', 'controller' => 'contents', 'action' => 'edit_upload',
