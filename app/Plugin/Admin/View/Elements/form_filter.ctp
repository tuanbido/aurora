...
<?php
if (count($adminConfig['fields']['filter']) > 1) {
	...
	$isAvanced = false;
	foreach ($adminConfig['fields']['filter'] as $field => $multiple) {
		if (!in_array($field, $adminConfig['fields']['hide']) and $field != 'adminHABTM') {
			$fieldType = $schema[$field]['type'];
			$params = array();
			$before = $after = '';
			if (array_key_exists($field, $adminConfig['fields']['filter_advanced'])) {
			...
			if (
				...
				or ($schema[$field]['class'] == 'number' and empty($adminConfig['fields']['filter'][$field]))
			) {
				if ($schema[$field]['class'] == 'date') {
					$params += array(
						'type' => $fieldType,
						'minYear' => $adminConfig['fields']['date_ranges'][$field]['minYear'],
						'maxYear' => $adminConfig['fields']['date_ranges'][$field]['maxYear'],
						'dateFormat' => $adminConfig['fields']['date_ranges'][$field]['dateFormat'],
						'monthNames' => $adminConfig['fields']['date_ranges'][$field]['monthNames'],
						'timeFormat' => '24',
						'empty' => '-',
						'separator' => '',
					);
				...
				echo $before . $this->Form->input(
					$model . '.' . $field . '_from',
					$params + array('label' => $adminConfig['fields']['names'][$field] . ' ' . __d('brownie', 'from', true))
				) . $this->Form->input(
					$model . '.' . $field . '_to',
					$params + array('label' => $adminConfig['fields']['names'][$field] . ' ' . __d('brownie', 'to', true))
				) . $after;
			} else {
				$params += array(
					'empty' => '-',
					'label' => __($adminConfig['fields']['names'][$field], true),
				);
	...
	foreach ($adminConfig['fields']['filter']['adminHABTM'] as $relatedModel) {
		$params = array(
			'empty' => false,
			'multiple' => 'multiple',
