<?php
	...
	<p class="counter">' . $this->Paginator->counter(array(
		'format' => CakeText::insert(
			...
			array('name_plural' => __($adminConfig['names']['plural']))
		),
		'model' => $model
	)) . '</p>';
	...
	$limit = $adminConfig['paginate']['limit'];
	$limits = '';
	if ($this->Paginator->params['paging'][$model]['pageCount'] > 1) {
		...
		foreach ($limitations as $i => $limit) {
			...
			if ($this->Paginator->params['paging'][$model]['count'] >= $prev) {
				...
				$limitCompare = (!empty($this->params['named']['limit'])? $this->params['named']['limit']:$adminConfig['paginate']['limit']);
				$limits .= '
	...
	if ($limits) {
		echo '
		<div class="limiter">
			<p>' . __d('brownie', '%s per page', __($adminConfig['names']['plural'])) . ':</p>
			<ul>' . $limits . '</ul>
		</div>';
	}
	echo '</div>';
