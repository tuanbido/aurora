...
*/
function adminResizeImage($file, $sizes) {
	if (!is_file($file)) {
		return false;
	}
	...
	switch ($ext) {
		...
		case 'jpg': case 'jpeg':
			...
			if (admin_image_fix_orientation($oldImage, $file)) {
	...
	$quality = Configure::read('adminSettings.defaultImageQuality');
	$saved = false;
	switch ($ext) {
		case 'gif':
...
function admin_image_fix_orientation(&$image, $filename) {
