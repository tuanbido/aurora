...
class ContentsController extends AdminAppController {
	...
	public $helpers = array('Admin.i18n');
	public $Model;
	...
	public function beforeFilter() {
		...
		$this->Model->Behaviors->attach('Admin.AdminPanel');
		...
		if (!$this->_adminCheckPermissions($model, $action)) {
			throw new NotFoundException('No permissions');
		}

		$this->Model->adminConfig['actions'] = array_merge(
			$this->Model->adminConfig['actions'],
			$this->arrayPermissions($this->Model->alias)
		);
		$this->_checkAdminUserCrud();
		$this->Content->i18nInit($this->Model);

		$adminConfig = $this->Model->adminConfig;
		$schema = $this->Content->schemaForView($this->Model);
		$model = $this->Model->alias;
		$this->set(compact('model', 'schema', 'adminConfig'));
	}


	public function index() {
		$this->Paginator->settings = $this->Model->adminConfig['paginate'];
		$this->Paginator->settings['fields'] = array_diff(
			$this->Model->adminConfig['paginate']['fields'],
			array_keys($this->Model->adminConfig['fields']['virtual'])
		);
		...
		if (method_exists($this->Model, 'adminAfterFind')) {
			$records = $this->Model->adminAfterFind($records);
		}
		$this->set(array(
			'records' => $this->_formatForView($records, $this->Model),
			'permissions' => array($this->Model->alias => $this->Model->adminConfig['actions']),
			'filters' => $this->_filtersForView($filters),
			'isAnyFilter' => !empty($filters),
		));
		if ($this->Model->adminConfig['fields']['filter']) {
	...
	public function view($model, $id) {
		...
		if (method_exists($this->Model, 'adminAfterFind')) {
			$record = $this->Model->adminAfterFind($record);
		}
		$record = $record[0];

		//ejecutar adminAfterFind en los modelos relacionados que estan en $contain
		...
		$assoc_models = array();
		if (!empty($this->Model->hasMany) and $this->Model->adminConfig['show_children']) {
			foreach ($assocs as $key_model => $related_model) {
				if (substr($key_model, 0, 8) == 'AdminI18n_') continue;
				if (!in_array($key_model, $this->Model->adminConfig['hide_children'])) {
					if ($key_model == $model) {
						$AssocModel = $this->Model;
					} else {
						$AssocModel = $this->Model->$key_model;
					}
					$AssocModel->Behaviors->attach('Admin.AdminPanel');
					if ($this->_adminCheckPermissions($key_model)) {
						if ($indx = array_search($related_model['foreignKey'], $AssocModel->adminConfig['paginate']['fields'])) {
							unset($AssocModel->adminConfig['paginate']['fields'][$indx]);
						}
						$filters = Hash::merge(
						...
						$this->Paginator->settings[$AssocModel->name] = Set::merge(
							$AssocModel->adminConfig['paginate'],
							array('conditions' => $filters),
							array('contain' => $this->Content->relatedModelsForIndex($AssocModel, $AssocModel->adminConfig['paginate']))
						);
						$this->Paginator->settings[$AssocModel->name]['fields'] = array_diff(
							$AssocModel->adminConfig['paginate']['fields'],
							array_keys($AssocModel->adminConfig['fields']['virtual'])
						);
						$assoc_models[] = array(
							'adminConfig' => $AssocModel->adminConfig,
							'model' => $key_model,
		...
		$this->set('adminConfig', $this->Model->adminConfig);
	...
	public function edit($model, $id = null) {
		...
		if (!empty($this->request->data)) {
			...
			if ($this->Content->makeSave($this->Model, $this->request->data, $fields)) {
				$msg =	($this->Model->adminConfig['names']['gender'] == 1) ?
					__d('brownie', 'The %s has been saved [male]', __($this->Model->adminConfig['names']['singular'])):
					__d('brownie', 'The %s has been saved [female]', __($this->Model->adminConfig['names']['singular']));
				...
				$msg =	($this->Model->adminConfig['names']['gender'] == 1) ?
					__d('brownie', 'The %s could not be saved. Please, check the error messages.[male]', __($this->Model->adminConfig['names']['singular'])):
					__d('brownie', 'The %s could not be saved. Please, check the error messages.[female]', __($this->Model->adminConfig['names']['singular']));
		...
		if (empty($this->request->data)) {
			if ($id) {
				$this->Model->Behaviors->attach('Containable');
				if ($this->Model->adminConfig['images']) {
					$contain[] = 'AdminImage';
				}
				if ($this->Model->adminConfig['files']) {
					$contain[] = 'AdminFile';
				}
				foreach ((array)$this->Model->schema() as $field => $cnf) {
					$this->Model->adminConfig['fields']['sanitize_html'][$field] = false;
				}
		...
		if (method_exists($this->Model, 'adminBeforeEdit') or !empty($this->Model->Behaviors->__methods['adminBeforeEdit'])) {
			$this->request->data = $this->Model->adminBeforeEdit($this->request->data);
	...
	public function delete($model, $id) {
		...
		$home = array('plugin' => 'admin', 'controller' => 'admin', 'action' => 'index', 'admin' => false);
	...
	public function delete_multiple($model) {
		$plural = $this->Model->adminConfig['names']['plural'];
	...
	public function edit_upload($model, $uploadType, $recordId, $categoryCode, $uploadId = null) {
		if (
			!in_array($uploadType, array('AdminFile', 'AdminImage'))
			or empty($this->Model->adminConfig[($uploadType == 'AdminFile') ? 'files' : 'images'][$categoryCode])
		) {
		...
		if (empty($this->request->data) and $uploadId) {
			...
			$uploadKey = ($uploadType == 'AdminFile') ? 'files' : 'images';
			$max = ($this->Model->adminConfig[$uploadKey][$categoryCode]['index'])? 1:10;
		}
	...
	public function delete_upload($model, $uploadType, $recordId) {
		if (!in_array($uploadType, array('AdminFile', 'AdminImage'))) {
			$this->response->statusCode('404');
		}
		...
		if (!$redirecTo) {
			$redirecTo = array('controller' => 'admin', 'action' => 'index', 'plugin' => 'admin');
		}
	...
	public function import($model) {
		if (!$this->Model->adminConfig['actions']['import']) {
			$this->response->statusCode('404');
		}
		if (!empty($this->request->data)) {
			$result = $this->Model->adminImport($this->request->data);
			if (is_array($result)) {
				$import = $result;
		...
		if (Configure::read('debug') and !method_exists($this->Model, 'adminImport')) {
			$msg = __d('brownie', 'Warning: %s::adminImport() must be defined', $model);
			$this->Session->setFlash($msg, 'flash_error');
		}
	...
	public function export($model) {
		$type = $this->Model->adminConfig['export']['type'];
		if (empty($type)) {
		...
		$this->set(array(
			...
			'relatedAdminConfig' => $this->Content->getRelatedAdminConfig($this->Model),
		));
	...
	public function reorder($model, $direction, $id) {
		if (
			...
			and empty($this->Model->adminConfig['sortable'])
		) {
			$this->response->statusCode('404');
		}
	...
	public function filter($model) {
		...
		foreach ($this->Model->adminSchema() as $field => $cnf) {
			$type = $cnf['type'];
			if (in_array($type, array('date', 'datetime'))) {
				...
				(
					$type == 'integer'
					and !$this->Content->adminIsForeignKey($this->Model, $field)
					and array_key_exists($field, $this->Model->adminConfig['fields']['filter'])
					and empty($this->Model->adminConfig['fields']['filter'][$field])
				)
			) {
				foreach (array('_from', '_to') as $key) {
					if (
		...
		if (!empty($retData[$Model->name])) {
			foreach ($retData[$Model->name] as $key => $value) {
				if (in_array($key, $fieldsHide)) {
					...
					if ($this->_adminCheckPermissions($RelModel->name, 'view', $data[$fK[$key]['alias']]['id'])) {
						$relatedURL = Router::url(array(
							'controller' => 'contents', 'action' => 'view', 'plugin' => 'admin',
			...
			$retData[$Model->name]['admin_actions'] = $this->Content->actions($Model, $data, $permissions);
		}
		return $retData;
	}
	...
	public function _formatDate($date) {
		if (empty($date) or $date == '0000-00-00') {
			...
			return date(Configure::read('adminSettings.dateFormat'), strtotime($date));
		}
	}
	...
	public function _formatDateTime($datetime) {
		if (empty($datetime) or $datetime == '0000-00-00 00:00:00') {
			...
			return date(Configure::read('adminSettings.datetimeFormat'), strtotime($datetime));
		}
	}
	...
	public function _setAfterSaveOptionsParams($Model, $data) {
		...
		if (!empty($this->params['named']['after_save'])) {
			...
			$default = 'referer';
		} elseif ($Model->adminConfig['actions']['view']) {
			$default = 'view';
		} elseif ($Model->adminConfig['actions']['index']) {
			$default = 'index';
		} else {
			$default = 'home';
		}
		...
		$params = array(
			...
			'options' => array(
				...
				'view' => ($Model->adminConfig['names']['gender'] == 1) ?
					__d('brownie', 'View saved %s [male]', __($Model->adminConfig['names']['singular'])):
					__d('brownie', 'View saved %s [female]', __($Model->adminConfig['names']['singular']))
				,
				'add' =>  ($Model->adminConfig['names']['gender'] == 1) ?
					__d('brownie', 'Add another %s [male]', __($Model->adminConfig['names']['singular'])):
					__d('brownie', 'Add another %s [female]', __($Model->adminConfig['names']['singular']))
				,
				'index' => ($Model->adminConfig['names']['gender'] == 1) ?
					__d('brownie', 'Go to to index of all %s [male]', __($Model->adminConfig['names']['plural'])):
					__d('brownie', 'Go to to index of all %s [female]', __($Model->adminConfig['names']['plural']))
				,
				'edit' => ($Model->adminConfig['names']['gender'] == 1) ?
					__d('brownie', 'Continue editing this %s [male]', __($Model->adminConfig['names']['singular'])):
					__d('brownie', 'Continue editing this %s [female]', __($Model->adminConfig['names']['singular']))
				,
		...
		foreach (array('add', 'view', 'index') as $action) {
			if (!$Model->adminConfig['actions'][$action]) {
				unset($params['options'][$action]);
			}
		}

		if ($Model->adminConfig['parent']) {
			$parentModel = $Model->{$Model->adminConfig['parent']};
			$params['options']['parent'] =	($parentModel->adminConfig['names']['gender'] == 1) ?
				__d('brownie', 'Go to the %s [male]', __($parentModel->adminConfig['names']['singular'])):
				__d('brownie', 'Go to the %s [female]', __($parentModel->adminConfig['names']['singular']));
		}
	...
	public function _setFilterData($Model) {
		$filterFields = $this->Model->adminConfig['fields']['filter'];
		$model = $Model->alias;
		foreach ($filterFields as $field => $multiple) {
			if ($field == 'adminHABTM') continue;
			$schema = $Model->adminSchema();
			$type = $schema[$field]['type'];
			$isRange = (
				in_array($type, array('date', 'datetime', 'float'))
				or
				(in_array($type, array('integer')) and !$this->Content->adminIsForeignKey($this->Model, $field) and !$multiple)
			);
			if ($isRange) {
				foreach (array('_from', '_to') as $key) {
		...
		foreach ($filterFields['adminHABTM'] as $relatedModel) {
		...
		foreach ($Model->hasAndBelongsToMany as $alias => $relatedModel) {
			if (in_array($relatedModel['className'], $filterFields['adminHABTM'])) {
				$relatedClassNames[] = $alias;
			}
		}
	...
	public function _checkAdminUserCrud() {
		$authModel = AuthComponent::user('model');
		$mustRedirect = false;
		if ($this->Model->alias == 'AdminUser') {
	...
	public function _hideConditionalFields($Model, $record) {
		...
		foreach ($Model->adminConfig['fields']['conditional'] as $field => $config) {
		...
		$Model->adminConfig['fields']['no_view']
			= array_merge($Model->adminConfig['fields']['hide'], $fieldsToHide);
		$Model->adminConfig['hide_related']['hasAndBelongsToMany']
			= array_merge($Model->adminConfig['hide_related']['hasAndBelongsToMany'], $habtmToHide);
	}
	...
	public function _afterSaveRedirect() {
		switch ($this->request->data['Content']['after_save']) {
			...
			break;
			case 'parent':
				if ($parent = $this->Model->adminConfig['parent']) {
