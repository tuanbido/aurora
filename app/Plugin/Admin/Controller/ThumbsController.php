...
class ThumbsController extends AdminAppController{
	...
	public function view($model = '', $recordId = '', $sizes = '', $category_code = '', $file = '') {
		...
		$AdminImage = ClassRegistry::init('AdminImage');
		$cachedFile = $AdminImage->resizedVersions($model, $recordId, $sizes, $category_code, $file);
		if (is_file($cachedFile)) {
			...
			if (method_exists($Model, 'adminAfterThumbnail')) {
				$Model->adminAfterThumbnail($recordId, $sizes, $category_code, $file);
			}
			exit;
		}
    }
