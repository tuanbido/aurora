...
function multipleComboSelect() {
	$('.combo-select').comboselect({ sort: 'none', addbtn: adminMsg.select + ' &raquo;',  rembtn: '&laquo; ' + adminMsg.unselect});
}

function bindFancyBox() {
	$('a.admin-image').fancybox({'titlePosition': 'inside'});
...
function checkMultiple() {
	...
	$('#deleteMultiple').submit(function(){
		...
		if (!allowSubmit) {
			alert(adminMsg.no_checked_for_deletion);
		}
		return allowSubmit;
	});
...
function showAdvancedFilters() {
	if ($('.advanced').length) {
		$('#filterSubmit').after('<a href="#" id="show_advanced">' + adminMsg.show_advanced + '</a>');
		$('#show_advanced').toggle(function(){
			$('.advanced').show();
			$(this).html(adminMsg.hide_advanced)
		}, function() {
			$('.advanced').hide();
			$(this).html(adminMsg.show_advanced)
		});
	}
}
	...
	$('#toggle-menu a').click(function() {
		$('#menu').slideToggle('fast', function(){
			if ($('#menu').is(':visible')) {
				...
				$('#toggle-menu a').attr('title', adminMsg.hide_menu);
			} else {
				$('#toggle-menu a').addClass('toggle-hidden');
				$('#toggle-menu a').attr('title', adminMsg.show_menu);
			}
		});
